...
let icon = document.querySelector(".floating-menu .icon-box");
...
window.onscroll = function () {
  ...
  for (let i = 0; i < experience.length; i++) {
    ...
    if (
      windowScrolled >
      time_line_offSet - windowHeight * 2.6 + time_line_Height + expoffSet
    ) {
...
menuBtn.onclick = function (e) {
  ...
  if (
    ...
    megaMenu.classList.contains("block")
  ) {
    setTimeout(function () {
...
document.addEventListener("click", (e) => {
  ...
  if (e.target !== menuBtn && e.target !== megaMenu) {
    ...
    if (
      megaMenu.classList.contains("block", "active") &&
